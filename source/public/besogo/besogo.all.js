    ...
    if (tool === "navOnly") {
      ...
      for (i = 1; i <= sizeX; i++) {
        for (j = 1; j <= sizeY; j++) {
          ...
          switch (tool) {
            case "auto":
              if (current.requestColor && current.moveNumber === 0) {
                element = besogo.svgStone(x, y, current.firstColor);
              } else {
...
besogo.makeGameRoot = function (sizeX, sizeY) {
  ...
  // Get next to move
  root.firstColor = BLACK;
  root.requestColor = false;
  root.nextMove = function () {
    ...
    if (this.lastMove) {
      ...
      if (this.requestColor && this.moveNumber === 0) {
        return this.firstColor; // Lấy màu quân cờ đầu tiên
      } else {
...
(function () {
  ...
  besogo.realStone = function (x, y, color, index) {
    ...
    color = window.besogoImagePath + "/" + color + ".png";
...
besogo.makeToolPanel = function (container, editor) {
  ...
  function makeButtonSVG(tool, tooltip, addClassBtn = null) {
    var button = document.createElement("button"),
      svg = besogo.svgEl("svg", {
        // Icon container
        width: "100%",
    ...
    button.title = tooltip;
    if (addClassBtn !== null) {
      button.classList.add(addClassBtn);
    }
